...
# important modules
use lib "lib/";
use Net::SILC; # 0.7 is not backwards compatible with 0.63 for CTCP responses
...
my ($server, $port, $password, $localAddr, @nicks, @channels, %channelKeys, 
    $owner, @ignoredUsers, @ignoredTargets);
my $nick = 0;
my $sleepdelay = 60;
...
my $uptime;
my $irc = new Net::SILC or 
  confess("Could not create a new Net::SILC object. Aborting");
...
sub connect {
    ...
    until (inet_aton($server) and # we check this first because 
				  #  Net::SILC::Connection doesn't
           $bot = $irc->newconn(
             Server => $server,
             Port => $port,
...
#        }
    ...
    #debug("Handlers added -- CONNECTING to server '".
    #       $bot->{user}->{server}."'");
    #$bot->connect($bot->{user}->{server},$bot->{user}->{port});
...
sub on_set_nick {
    ...
    print "*MOZBOT On Set Nick\n";

    my ($self, $event) = @_;

    #print "::MOZBOT::on_set_nick:\n", Data::Dumper->Dump([\$event]), "\n";
    ...
    # set variable
    $nick = $newnick;
    &debug("on_set_nick: using nick '$nicks[$nick]'");

    &debug("* SKIPPING WHOIS CALL *");
...
sub on_connect {
    ...
    # XXY ** HACK!! ** - math -
    # call on_set_nick bceause SILC doesnt supply a welcome (or motd FTM).
    &debug("*** HACK! Manually calling on_set_nick before connect.");
    &on_set_nick($self, $ev);
    &debug("*** HACK! Back to on_connect from manual on_set_nick call.");
...
&debug("* About to create admin module.");

    @modules = (BotModules::Admin->create('Admin', '')); # admin commands

&debug("* Created admin module.");
...
&debug ("* Attempting to load $_");
    ...
    &debug("Telling modules they're on irc ;)");
   ...
   &debug("Joining channels: '". join(",",@channels)."'");
    ...
    # all done!
    &debug('initialization took '.&days($uptime).'.');
    $uptime = time();
}
...
sub bot_select {
    ...
    local $/;
    undef $/;
    &debug("child ${$pipe}->{'BotModules_PID'} slurping ${$pipe}->{'BotModules_ChildType'}");
    my $data = <$pipe>;
...
sub create {
    ...
    &::debug("mozbot: creating module name '$name' filename '$filename'");
    my $self = {
        '_name' => $name,
    ...
    &::debug("mozbot: about to initialize module.");
    $self->Initialise();

    &::debug("mozbot: registering config for module $name");
    $self->RegisterConfig();
    &::debug("mozbot: config registered. returning.");
    return $self;
}
...
sub registerVariables {
    ...
    &::debug("registerVariables: starting to register.");

    foreach (@variables) {

#print $$_[0], " = ", Data::Dumper->Dump([\$$_[3]]), "\n";
...
print "MOZBOT: setting nick '$value'\n";
...
sub RegisterConfig {
    ...
    &::debug("RegisterConfig(): registering Variables.");
    ...
    &::debug("RegisterConfig(): registered. Returning.");
    return {};
}
...
sub RegisterConfig {
    ...
    &::debug("Admin::RegisterConfig: calling main RegisterConfig()...");
    $self->SUPER::RegisterConfig(@_);

    &::debug("Admin::RegisterConfig: done main Register. Doing custom.");
    
print "="x78, "\n";
    &::debug("Registering constants: \$0 is '$0'");
    $self->registerVariables(
      # [ name, save?, settable?, value ]
        ['allowChannelAdmin', 1, 1, 0], # default: no admin from a channel
        ['sourceCodeCheckDelay', 1, 1, 20], # default: 20s tween src checks

        # files to check for source code changes
        ['files', 1, 1, [$0, 'lib/Mails.pm', 'lib/Configuration.pm', 
         'lib/IO/SecurePipe.pm']], 

        ['channels', 0, 0, undef], # remove the 'channels' internal variable...
        ['autojoin', 0, 0, 0], # remove the 'autojoin' internal variable...
        ['errorMessagesMaxLines', 1, 1, 5], # default, only 5 lines in errmsg
        ['allowInviting', 1, 1, 1], #default: anyone can invite bot to chan
    );

    &::debug("Admin::RegisterConfig: registered constants. Now config file.");
    # now add in all the global variables...
    foreach (keys %configStructure) {
        # &::debug("Admin::RegisterConfig: registering config for $_");
        $self->registerVariables([$configStructure{$_}[0], 0, 1, $configStructure{$_}[1]]) if (ref($configStructure{$_}[1]) =~ /^(?:SCALAR|ARRAY|HASH)$/go);
    }
    &::debug("Admin::RegisterConfig: done w/config file vars. Returning.");
}
